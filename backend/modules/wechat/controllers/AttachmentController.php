...
use wayfiretech\netos\common\models\wechat\Attachment;
use wayfiretech\netos\common\models\wechat\News;
use wayfiretech\netos\backend\modules\wechat\models\NewsPreview;
...
class AttachmentController extends WController
{
    ...
    public function actionNewsIndex()
    {
        ...
        $statusl = Yii::$app->request->get('status');
        $status =  !isset($statusl) || empty($statusl)?2:$statusl;
        
        

        $attachment = $status == 2 || !isset($status)?Attachment::getList($mediaType):Attachment::getList($mediaType,$status);

        $request_url  = Yii::$app->request->hostInfo;
      
        foreach ($attachment as $key => &$value) {
            $value['attachment'] =  $request_url.$value['attachment'];
            foreach ($value['news'] as $ke => &$val) {
                # code...
              $val['thumb_url'] =  $request_url.$val['thumb_url'];
    ...
    public function actionNewsEdit()
    {
        ...
        {   
            ...
            if ($status==2) {//本地服务器
                ...
                foreach ($list as $key => &$item)
                {
                    ...
                    $attachment->media_id = Attachment::random_string(6).time();
                    ...
                    $attachment->attachment =str_replace($prefix,'',trim($thumb_url));
                    ...
                    $news = new News();
                    $news->attributes = $vo;
                    $news->title = trim($item['title']);
                    $news->content = trim($item['content']);
                    $news->thumb_media_id = $attachment->media_id;
                    $news->content_source_url = trim($item['content_source_url']);
                    $news->show_cover_pic = trim($item['show_cover_pic']);
                    $news->digest = trim($item['digest']);
                    $news->author = trim($item['author']);
                    $news->status = $status;
                    $news->thumb_url = trim($item['thumb_url']);

                    
                    $news->url = AddonsUrl::toAddonsUrl(['wxarticle/index'],array('addon'=>'website','id'=>$attr_id));
            ...
            foreach ($list as $key => &$item)
            {
                ...
                if(strpos(urldecode($item['thumb_url']),$this->_wechaMediatUrl) === false)
                {
                    ...
                    if ($image_material['errcode'] !=200) {
                            $result = $this->setResult();
                            $result->code = 0;
                            $result->message = $image_material['errmsg'].$status;
                            return $this->getResult();
                    }
        ...
        foreach ($attachment as $key => &$value) {
            if ($key == 'attachment') {
                $attachment[$key] = $request->hostInfo.$value;
            }
        }
    ...
    public function actionImageAdd()
    {
        ...
        if ($model->load(Yii::$app->request->post()))
        {
            if($model->attachment)
            {
                ...
                if ($image_material['errcode'] != 0) {//没有成功
                    
                     return $this->message("上传失败，".$image_material['errmsg'],$this->redirect(['index']));
