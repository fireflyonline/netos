...
use wayfiretech\netos\common\models\wechat\RuleStat;
use wayfiretech\netos\common\models\wechat\RuleKeywordStat;
use wayfiretech\netos\common\models\wechat\FansStat;
...
class StatController extends WController
{
    ...
    {
        // 更新微信统计数据
        FansStat::getFansStat($this->_app);
        ...
        $yesterdayModel = FansStat::find()
            ->where(['append' => strtotime(date('Y-m-d')) - 60*60*24])
            ->asArray()
            ->one();
        ...
        $todayModel = FansStat::find()
            ->where(['append' => strtotime(date('Y-m-d'))])
            ->asArray()
            ->one();
    ...
    public function actionRule()
    {
        ...
        $data = RuleStat::find()
            ->select(['rule_id','sum(hit) as hit','max(updated) as updated'])
            ->groupBy(['rule_id'])
            ->where(['between','append',strtotime($from_date),strtotime($to_date)]);
    ...
    public function actionRuleKeyword()
    {
        ...
        $data = RuleKeywordStat::find()
            ->select(['keyword_id','sum(hit) as hit','max(rule_id) as rule_id','max(updated) as updated'])
            ->groupBy(['keyword_id'])
            ->where(['between','append',strtotime($from_date),strtotime($to_date)]);
